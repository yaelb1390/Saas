@props(['title', 'action', 'form', 'method' => 'PUT', 'trigger' => 'Editar'])

{{-- Modal de edición reutilizable. Como el de alta, pero envía por PUT/PATCH y su disparador es un
     enlace discreto (no el botón primario «+»). Se reabre solo si su propio formulario tuvo errores
     de validación (cada modal usa un «_form» único). --}}
<div x-data="{ open: {{ old('_form') === $form ? 'true' : 'false' }} }" class="inline-block">
    <button type="button" @click="open = true" class="bmos-btn bmos-btn-ghost">
        {{ $trigger }}
    </button>
