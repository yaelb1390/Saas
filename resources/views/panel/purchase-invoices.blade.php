                ...
                <tbody>
                    @forelse ($invoices as $inv)
                        <tr>
                            ...
                            <td class="text-right">
                                @can('purchase_invoices.manage')
                                    <form method="POST" action="{{ route('panel.purchase-invoices.destroy', $inv) }}"
                                          onsubmit="return confirm('¿Eliminar esta factura de compra?')" class="inline">
                                        @csrf @method('DELETE')
                                        <button type="submit" class="text-rose-500 hover:text-rose-700" title="Eliminar">
