                                    ...
                                    <td class="text-right">
                                        ...
                                        @can('purchases.receive')
                                            @if ($order->status->canBeReceived())
                                                {{-- No borra nada, pero es de las de mayores consecuencias
                                                     contables de todo el panel: sube la existencia y no hay
                                                     botón de deshacer. Por eso el aviso, aunque el tono no
                                                     sea el rojo de los borrados. --}}
                                                <x-panel.confirm-action
                                                    :action="route('panel.purchase-orders.receive', $order)"
                                                    method="POST"
                                                    tone="neutral"
                                                    title="¿Dar por recibida la orden {{ $order->code }}?"
                                                    message="Sumará al almacén la existencia de todas sus líneas y el costo pasará a contar en el inventario."
                                                    note="No hay botón para deshacerlo: si la mercancía no llegó completa, habría que ajustar la existencia a mano."
                                                    irreversible
                                                    confirm="Sí, la recibí"
                    ...
                    @forelse ($suppliers as $supplier)
                        <div class="flex items-center justify-between p-4">
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <x-panel.confirm-action
                                    :action="route('panel.suppliers.destroy', $supplier)"
                                    title="¿Eliminar a «{{ $supplier->name }}»?"
                                    message="Se archiva y deja de ofrecerse al crear órdenes de compra."
                                    note="Sus órdenes y facturas anteriores se conservan tal cual."
                                    tooltip="Eliminar"
